...
DWORD WINAPI ThreadProc(CONST LPVOID lpParam)
{
	...
	for (unsigned int i = threadParams->startRow; i < threadParams->endRow; ++i)
	{
		for (unsigned int j = 0; j < g_width; ++j)
		{
			...
			*threadParams->outStream << ((float)elapsedTime / CLOCKS_PER_SEC) << endl;
		}
	}
	ExitThread(0);
}
...
RGBQuad** Blur(RGBQuad** rgbInfo, BitMapInfoHeader& fileInfoHeader, int threadsAmount, int coresAmount, vector<ThreadPriority> threadsPriority)
{
	...
	ofstream* outThreadsStreams = new ofstream[threadsAmount];
	...
	for (int i = 0; i < threadsAmount; i++)
	{
		...
		outThreadsStreams[i] = ofstream("t" + to_string(i + 1) + ".txt");

		threadParams[i] = { startRow, endRow, i, &outThreadsStreams[i] };
...
int main(int argc, char *argv[])
{
	...
	{
		cout << "Usage: lw4.exe infile_name outfile_name threads_amount cores_amount priorities(separated by space)\n Example priorities:\n below_normal(-1) \n normal(0) \n above_normal(1)";
		return 0;
	}
