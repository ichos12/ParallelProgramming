...
RGBQuad** Blur(RGBQuad** rgbInfo, BitMapInfoHeader& fileInfoHeader, int threadsAmount, int coresAmount)
{
	...
	for (int i = 0; i < threadsAmount; i++)
	{
		...
		threadParams[i] = { startRow, endRow };
...
{
	if (argc < 4) {
		cout << "Usage: lw2.exe infile_name outfile_name threads_amount cores_amount" << endl;
		return 0;
	}
	...
	RGBQuad** blurredImg = Blur(inputImg, fileInfoHeader, threadsAmount, coresAmount);
	
	writeBitMap(outFileName, fileHeader, fileInfoHeader, blurredImg);
	clock_t finishTime = clock();

	cout << finishTime - startTime << " ms" << endl;
	return 0;
}